...
async function displayWeather(lat, lon, city, state) {
  ...
  try {
    ...
    const dateSec = document.getElementById("date_time");
    dateSec.innerHTML = result.current.last_updated;

    //display location
    const locationSec = document.getElementById("location");
    locationSec.innerHTML = `${city}, ${state}`;

    //display weather condition
    const weatherSec = document.getElementById("weather");
    weatherSec.innerHTML = result.current.condition.text;

    //display weather icon
    const iconSec = document.getElementById("icon");
    const iconUrl = result.current.condition.icon;
    iconSec.innerHTML = `<img src="${iconUrl}" alt="Weather Icon">`;

    //display temperature in F by default
    const tempSec = document.getElementById("temperature");
    tempSec.innerHTML = `${Math.round(result.current.temp_f)}°F`;
...
function changeUnits(result) {
  const tempSec = document.getElementById("temperature");
  if (tempSec.innerHTML.includes("°F")) {
    tempSec.innerHTML = `${Math.round(result.current.temp_c)}°C`;
  } else {
    tempSec.innerHTML = Math.round(result.current.temp_f) + "°F";
  }
}
