...
async function displayWeather(lat, lon, city, state) {
  ...
  try {
    ...
    //add event listener to unit toggle
    document
      .getElementById("unit-toggle")
      .addEventListener("change", () => changeUnits(result));
  } catch (e) {
    console.log("error:", e);
  }
}
//function to change units from F to C and vice versa when box checked
function changeUnits(result) {
  const temp = document.getElementById("temperature");
  const units = document.getElementById("unit-toggle");
  if (units.checked) {
    temp.innerHTML = `${Math.round(result.current.temp_c)}°C`;
  } else {
    temp.innerHTML = `${Math.round(result.current.temp_f)} °F`;
  }
}
